...
/**
 * Gallery Landing Page Custom Post Type and Fields
 */

// Register Custom Post Type for Gallery Images
function create_gallery_image_post_type() {
    $labels = array(
        'name'                  => 'Gallery Images',
...
add_action('init', 'create_gallery_image_post_type');

// Add custom fields for gallery images
function add_gallery_image_meta_boxes() {
    add_meta_box(
        'gallery_image_details',
        'Gallery Image Details',
        'gallery_image_meta_box_callback',
        'gallery_image',
        'normal',
        'high'
    );
}
add_action('add_meta_boxes', 'add_gallery_image_meta_boxes');

// Meta box callback function
function gallery_image_meta_box_callback($post) {
    wp_nonce_field('gallery_image_meta_box', 'gallery_image_meta_box_nonce');
    
    $image_url = get_post_meta($post->ID, '_gallery_image_url', true);
    $image_alt = get_post_meta($post->ID, '_gallery_image_alt', true);
    $image_caption = get_post_meta($post->ID, '_gallery_image_caption', true);
    
    echo '<table class="form-table">';
    echo '<tr>';
    echo '<th><label for="gallery_image_url">Image URL (optional - will use featured image if empty)</label></th>';
    echo '<td><input type="url" id="gallery_image_url" name="gallery_image_url" value="' . esc_attr($image_url) . '" class="regular-text" /></td>';
    echo '</tr>';
    echo '<tr>';
    echo '<th><label for="gallery_image_alt">Alt Text</label></th>';
    echo '<td><input type="text" id="gallery_image_alt" name="gallery_image_alt" value="' . esc_attr($image_alt) . '" class="regular-text" /></td>';
    echo '</tr>';
    echo '<tr>';
    echo '<th><label for="gallery_image_caption">Caption</label></th>';
    echo '<td><textarea id="gallery_image_caption" name="gallery_image_caption" rows="3" class="large-text">' . esc_textarea($image_caption) . '</textarea></td>';
    echo '</tr>';
    echo '</table>';
    
    echo '<p><strong>Note:</strong> Set a featured image for this gallery item. The featured image will be used if no custom URL is provided.</p>';
}

// Save meta box data
function save_gallery_image_meta_box_data($post_id) {
    if (!isset($_POST['gallery_image_meta_box_nonce'])) {
        return;
    }
    
    if (!wp_verify_nonce($_POST['gallery_image_meta_box_nonce'], 'gallery_image_meta_box')) {
        return;
    }
    ...
    if (isset($_POST['gallery_image_url'])) {
        update_post_meta($post_id, '_gallery_image_url', sanitize_url($_POST['gallery_image_url']));
    }
    
    if (isset($_POST['gallery_image_alt'])) {
        update_post_meta($post_id, '_gallery_image_alt', sanitize_text_field($_POST['gallery_image_alt']));
    }
...
add_action('save_post', 'save_gallery_image_meta_box_data');

// Add custom columns to admin list
function add_gallery_image_admin_columns($columns) {
    $new_columns = array();
...
add_filter('manage_gallery_image_posts_columns', 'add_gallery_image_admin_columns');

// Populate custom columns
function populate_gallery_image_admin_columns($column, $post_id) {
    switch ($column) {
        case 'thumbnail':
            ...
            break;
        case 'caption':
            $caption = get_post_meta($post_id, '_gallery_image_caption', true);
            echo $caption ? esc_html($caption) : '<span style="color: #999;">No caption</span>';
            break;
        case 'menu_order':
...
add_action('manage_gallery_image_posts_custom_column', 'populate_gallery_image_admin_columns', 10, 2);

// Make menu_order sortable
function make_gallery_image_sortable($columns) {
    $columns['menu_order'] = 'menu_order';
    return $columns;
}
add_filter('manage_edit-gallery_image_sortable_columns', 'make_gallery_image_sortable');

// Set default order by menu_order
function set_gallery_image_default_order($query) {
    if (is_admin() && $query->get('post_type') === 'gallery_image' && !$query->get('orderby')) {
        $query->set('orderby', 'menu_order');
        $query->set('order', 'ASC');
    }
}
add_action('pre_get_posts', 'set_gallery_image_default_order');
